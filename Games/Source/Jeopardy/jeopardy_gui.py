...
# the documentation, and a simple demonstration can be found in trivia.py. An internet connection
# is required for the program to work. Note that if a token file is present, the program will 
# overwrite the old token with the new one. Logs are stored in ./assets/logs.
# 
...
class MyLayout(GridLayout):
    ...
    def __init__(self, **kwargs) -> None:
        ...
        self.logger = DebugLogger(class_name="KivyLayout")
    ...
    def checkPreviousWinner(self) -> str:
        ...
        self.logger.log("Entering checkPreviousWinner...",for_debug=True)
        
        # get a list of all log files
        log_files = glob("assets/logs/log_*")
        if not log_files:
            self.logger.log("No previous logs found.",message_tag="WARNING")
            self.logger.log("Exiting checkPreviousWinner.",for_debug=True)
            return ""
        ...
        try:
            ...
            self.logger.log(f"Last log was not long enough: {e}",for_stderr=True)
            return ""

        # read the latest log file
        self.logger.log(f"Reading from file '{last_log_file}'")
        try:
            with open(last_log_file, 'r') as file:
                lines = file.readlines()
        except Exception as e:
            self.logger.log(f"Error reading log file: {e}",for_stderr=True)
        ...
        if not lines:
            self.logger.log("No log entries found.",message_tag="WARNING")
            self.logger.log("Exiting checkPreviousWinner.",for_debug=True)
            return ""
        ...
        try:
            ...
            self.logger.log(f"Error: {e}",for_stderr=True)
            self.logger.log(format_exc())
            self.logger.log("Exiting checkPreviousWinner.",for_debug=True)
            return ""
        ...
        # note that the fourth-to-last entry should be: Game ended. (team#) team wins! 
        #                                                           OR
        #                                               Game ended. (tie) It's a tie!
        if win_match:
            self.logger.log(f"Previous winner found to be team{win_match.group(1)} ({win_match.group(2)})")
            self.logger.log("Exiting checkPreviousWinner.",for_debug=True)
            return f"team{win_match.group(1)}"
        elif match(r"Game ended\. \(tie\) It's a tie!", last_line):
            self.logger.log("Previous game outcome found to be a tie")
            self.logger.log("Exiting checkPreviousWinner.",for_debug=True)
            return "tie"
        else:
            self.logger.log("No valid winner or outcome found in the log.",message_tag="WARNING")
            self.logger.log("Exiting checkPreviousWinner.",for_debug=True)
            return ""
    ...
    def initPopup(self, filename: Optional[str] = None) -> None:
        ...
        self.logger.log("Exiting initPopup.",for_debug=True)
    ...
    def changeAudio(self,
        ...
        self.logger.log(f"Use audio: {self.use_audio}")
    ...
    def saveTeamNames(self,
        ...
        self.logger.log("Entering saveTeamNames...",for_debug=True)
        ...
        self.API_TOKEN = JeopardyAPI(self.token_filename) if self.token_filename else JeopardyAPI()
        
        self.logger.log("Exiting saveTeamNames.",for_debug=True)
        self.mainWindow()
    
    # pre-condition: 
    # post-condition: 
    def mainWindow(self) -> None:
        self.logger.log("Entering mainWindow...",for_debug=True)
        ...
        self.logger.log(f"Categories: {selected_category_names} (indices {selected_category_indices})")
        ...
        self.logger.log("Exiting mainWindow.",for_debug=True)
    ...
    def press(self,
        ...
        self.logger.log("Entering press...",for_debug=True)
        ...
        self.logger.log(f"Pressed {current_question_points} in category {category} (index {category_index})")
        ...
        self.logger.log("Exiting press.",for_debug=True)
        self.displayQuestion(question_button)
    
    # pre-condition: 
    # post-condition: 
    def displayQuestion(self, question_button: Button) -> None:
        self.logger.log("Entering displayQuestion...",for_debug=True)
        self.logger.log(f"Current turn: {self.current_team}")
        ...
        if self.is_steal:
            ...
            self.logger.log(f"Steal opportunity for {self.current_team.name}!")
        ...
        self.logger.log("Exiting displayQuestion.",for_debug=True)
    ...
    def checkAnswer(self,
        ...
        self.logger.log("Entering checkAnswer...",for_debug=True)
        ...
        if selected_answer == self.correct_answer:
            ...
            # 
            self.logger.log(f"{self.current_team} is incorrect, the other team can now steal")
        ...
        self.logger.log("Exiting checkAnswer.",for_debug=True)
    
    # pre-condition: 
    # post-condition: 
    def removeButton(self, question_button: Button) -> None:
        self.logger.log("Entering removeButton...",for_debug=True)
        ...
        self.logger.log("Exiting removeButton.",for_debug=True)
    
    # pre-condition: 
    # post-condition: 
    def switchTurn(self) -> None:
        self.logger.log("Entering switchTurn...",for_debug=True)
            
        # swap to the opposite team
        self.current_team = self.team2 if self.current_team == self.team1 else self.team1
        self.logger.log(f"Next turn: {self.current_team}")
        ...
        self.logger.log("Exiting switchTurn.",for_debug=True)
    ...
    def playNoise(self,
        ...
        self.logger.log("Entering playNoise...",for_debug=True)
        ...
        if is_correct: sound_file += 'correct.mp3'
            ...
            self.logger.log("Error: no sound file specified", for_stderr=True)
            return
        ...
        self.logger.log("Exiting playNoise.",for_debug=True)

    # pre-condition: full or user-ended game
    # post-condition: 
    def endGame(self) -> None:
        self.logger.log("Entering endGame...",for_debug=True)
        ...
        if all_used:
            ...
            self.logger.log("All questions have been used. Ending the game.")
            self.game_over = True
            ...
            else: self.logger.log(f"Game ended. (team{team_num}) {winner_text}")
            ...
            self.logger.log("Exiting endGame.",for_debug=True)
        else: # this should not be hit
