 ...
 * Created: 10.3.23
 * Doctored: 5.29.2024
 * ~/csc4310/midterm/matrixCminus.cpp
 * 
 * 
 ...
 * 
 * To compile:
 * g++ s_matrixMult_2.cpp -Wall -o s_matrixMult_2
 * 
 * To run:
 * ./s_matrixMult_2 [matrix1 datafile] [matrix2 datafile] [output file]
 *
 * 
...
int main(int argc, char** argv)
{
    ...
    if(argc != 4) {
        cerr << "error: must have 4 arguments, " << argc << " provided.\nUsage: ./matrixCminus "
             << "[matrix1 datafile] [matrix2 datafile] [output file]" << endl;
        exit(1);
    }
...
void mult(const int** a, const int** b, int** product, const int& dim)
{
    // row
    for(int i=0; i<dim; i++)
        // column
...
void printMatrix(const string& outputFile, const int** array, const int& dim)
{
    ...
    if(!ans) {
        cerr << "matrix file '" << outputFile << "' unable to be opened" << endl;
        exit(2);
    }
