 ...
 * calculation is done using MPI. The output is displayed for the user and is also output to a
 * separate file.
 * 
 * The master process uses a modified version of the Strassen algorithm to recursively carve up the
 * matrix data at the top level into 8 multiplications, then uses OpenMPI to distribute to 8 worker
 * processes. Each worker performs the multiplications of the data it receives from the master. The
 * worker process then sends its solution back to the master. The master process calculates the
 * final product array and is output to a file and/or to the screen.
 * 
 * 
 * To compile:
 ...
 * 
 * To run:
 * mpiexec --mca btl_tcp_if_include <ethernet_ID> -n 4 -hostfile actHosts [matrix1 datafile]
 *     [matrix2 datafile] [output file]
 * 
 * 
 ...
 * 
 * 2 - number of MPI tasks is not divisible by 4
 * 
 * 3 - matrix data files unable to be opened
 * 
 * 4 - invalid matrix dimensions (incompatible or less than 1)
*/
...
void mult(const int**, const int**, int**, const int&);
...
int main(int argc, char** argv)
{
    ...
    if(argc != 11) {
        cerr << "error: must have 11 arguments, " << argc << " provided.\nUsage: mpiexec "
                << "--mca btl_tcp_if_include <ethernet_ID> -n 4 -hostfile actHosts [matrix1 "
                << "datafile] [matrix2 datafile] [output file]" << endl;
        exit(1);
    }
    ...
    if (numtasks % 4 != 0) {
        cerr << "Error: number of MPI tasks must be divisible by 4.\n";
        // quit MPI
    ...
    // unified dimension variable, defined here so it can be accessed by all processes
    int dim = INT_MIN;
    // same for these, defined here to avoid scope issues
    // first matrix
    int** a = INT_MIN;
    // second matrix
    int** b = INT_MIN;
    ...
    if(taskid == 0) {
        ...
        if(!matrixFile1) {
            cerr << "first matrix file '" << file1 << "' unabled to be opened\n";
            // quit MPI
            MPI_Abort(MPI_COMM_WORLD,2);
            exit(3);
        }
        if(!matrixFile2) {
            cerr << "second matrix file '" << file2 << "' unabled to be opened\n";
            // quit MPI
            MPI_Abort(MPI_COMM_WORLD,2);
            exit(3);
        }
        ...
        // unified dimension variable because IT SHOULD BE SQUARE, DIMS SHOULD BE THE SAME
        dim = dim1;
...
void mult(const int** A, const int** B, int** C, const int& dim)
{
    if(dim == 1)
        C[0][0] = A[0][0] * B[0][0];
    else {
        ...
        // I can't wait for you to tell me there's an easier solution/way to do this
        ...
        // if one the the parameters on this is wrong I'm gonna be upset
