 ...
 * multiplication. The output is displayed for the user and is also output to a
 * separate file.
 * 
 * 
 * To compile:
 * g++ matrixCminus.cpp -Wall -o matrixCminus
 * 
 * To run:
 * ./matrixCminus [matrix1 datafile] [matrix2 datafile] [output file]
 *
 * 
...
void mult(int** a, int** b, int** product, int dim)
{
    // row
    for(int i=0; i<dim; i++)
        // column
...
int main(int argc, char** argv)
{
    ...
    if(argc != 4) {
        cerr << "error: must have 4 arguments, " << argc << " provided. check "
             << "opening comment.\n" << endl;
        exit(1);
    }
