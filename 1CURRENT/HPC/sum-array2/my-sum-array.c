...
/* master_node_process - init an array of data to process Send on portion of
*                        the array to other cluster nodes. Keep one chunk for
*                        the master. Find the sum of the data. Receive the
*                        results from each node.
* precondition: chunksize, numtasks, and taskid are established in the main for
*               all processes
...
/* cluster_node_process - receive a chunk of data from the master find the sum
*                         of the data send back the data and sum. NOTE: there
*                         is no reason to send the data back, just demo how to
*                         send the array back.
* precondition: chunksize and taskid are established in the main for all
*               processes
* postcondition: nothing is returned from the function.
*/
void cluster_node_process(int chunksize, int taskid)
...
/* update - output the first 5 numbers each chunk of data passed to the
*           non-master processes
* precondition: The data array contains a collection of doubles chunksize is
*               the number of doubles in each set of values. numtasks is the
*               number of tasks (or number of subsets) that process the data.
