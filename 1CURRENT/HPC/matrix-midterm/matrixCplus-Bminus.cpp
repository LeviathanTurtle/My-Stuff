 ...
 * multiplication. The calculation is done using OpenMP. The output is
 * displayed for the user and is also output to a separate file.
 * 
 * To compile:
 * g++ matrixCplus-Bminus.cpp -Wall -fopenmp -o matrixCplus-Bminus
 * 
 * To run:
 * ./matrixCplus-Bminus [matrix1 datafile] [matrix2 datafile] [output file] [num threads]
 * 
 * 
...
void mult(int** a, int** b, int** product, int dim, int T)
{
...
int main(int argc, char** argv)
{
    ...
    if(argc != 5) {
        cerr << "error: must have 5 arguments, " << argc << " provided. check "
             << "opening comment.\n" << endl;
        exit(1);
    }
    ...
    // note: not ensuring product is array of 0's because that is handled in
    // mult function
