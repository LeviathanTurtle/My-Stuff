/*
    ...
    nvcc matrixMultEnd.cu -o [any name you want to use, i used matrixMult in my testing]
    
    execute code
    
    ./matrixMultEnd <tilewidth> <input.txt> <input2.txt> <output.txt> 
*/
...
void MatrixMulKernel(float *M, float *N, float *P, int Width, int TILE_WIDTH){
    ...
    for (int m = 1; m <= Width/TILE_WIDTH; ++m) {
        ...
        for (int k = 0; k < TILE_WIDTH; k += 4) {
            ...
            Pvalue += subTileM[ty * TILE_WIDTH + k + 1] * subTileN[(k + 1) * TILE_WIDTH + tx];
            Pvalue += subTileM[ty * TILE_WIDTH + k + 2] * subTileN[(k + 2) * TILE_WIDTH + tx];
            Pvalue += subTileM[ty * TILE_WIDTH + k + 3] * subTileN[(k + 3) * TILE_WIDTH + tx];
        }
